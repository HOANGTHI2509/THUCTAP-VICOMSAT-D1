...
print("=== BẮT ĐẦU QUY TRÌNH TIỀN XỬ LÝ DỮ LIỆU TỪ THƯ MỤC 'fulltt' ===")

def process_fulltt_file(input_file, out_dir):
    ...
    vehicle_id = os.path.basename(input_file).replace('_da_gop.xlsx', '').replace('.xlsx', '')
...
if __name__ == '__main__':
    input_dir = "D:/THUCTAP_VICOMSAT/fulltt"
    output_dir = "D:/THUCTAP_VICOMSAT/TienXuLy"
    
    os.makedirs(output_dir, exist_ok=True)
    
    xls_files = glob.glob(os.path.join(input_dir, "*_da_gop.xlsx"))
    print(f"Tìm thấy {len(xls_files)} files trong {input_dir}")
    
    for f in xls_files:
        process_fulltt_file(f, output_dir)
