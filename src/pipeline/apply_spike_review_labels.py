...
def apply_reviews(data_dir, candidate_path, review_path):
    ...
    for review in reviews:
        if str(review.get("decision", "")).upper() != "ACCEPT":
            continue
        if str(review.get("final_label", "")).strip() != "SPIKE":
            continue
        try:
        ...
        for row_index in _parse_source_rows(candidates[event_id].get("source_rows")):
            override_by_row[row_index] = "SPIKE"
    ...
    print(f"Applied spike review labels to rows: {len(override_by_row):,}")
    print(f"Changed labels: {changed:,}")
    print("Updated all_labeled_points.csv, train.csv, val.csv, test.csv")


def main():
    parser = argparse.ArgumentParser(description="Apply spike event review labels to dataset CSVs.")
    parser.add_argument("--data-dir", default=r"D:\THUCTAP_VICOMSAT\data\fuel_label_dataset")
    parser.add_argument("--candidates", default=r"D:\THUCTAP_VICOMSAT\data\fuel_label_dataset\spike_event_candidates.csv")
    parser.add_argument("--review", default=r"D:\THUCTAP_VICOMSAT\data\fuel_label_dataset\spike_event_review.csv")
